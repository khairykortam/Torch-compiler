...
from parser import *
from compiler_generator import *
from common import *
...
def usage(compiler_name):
    print("USAGE: torch [OPTIONS] <SUBCOMMAND> [ARGS]")
    print("    OPTIONS:")
    print("idk")
    ...
    if subcommand == "com":
        ...
        while len(argv) > 0:
            ...
            if arg == '-r':
                ...
                if len(argv) == 0:
                    usage(compiler_name)
                    print("[ERR] no arg is provided for -o", file=sys.stderr)
                    exit(1)
                out_path, *argv = argv
            else:
                program_path = arg
        ...
        if out_path is not None:
            if path.isdir(out_path):
                ...
                if basename.endswith(TORTH_EXT):
        ...
        program = Program(
            ops=parse_context.ops, memory_capacity=parse_context.memory_capacity
        )
